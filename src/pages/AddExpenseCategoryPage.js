...
import ExpenseTypesList from '../components/ExpenseTypesList/ExpenseTypesList';
...
function AddExpenseCategoryPage(props) {
  ...
  const handleExpenseTypeRemove = (expenseType) => {
    ...
    const canRemoveCategory = !expenseTypes.some(
      (exp) => exp.category === expenseType.category
    );
  ...
  return (
    <div className='container'>
      ...
      <div className='add-expense-category'>
        {showSuccessMessage && (
          <div className='success'>Action successful!</div>
        )}
        {showErrorMessage && (
          <div className='fail'>
        ...
        <div className='add-expense-category-input'>
          <label htmlFor='add-expense-type-input'>
            Write down a new expense name
          </label>
          <input
            type='text'
          ...
          {!preferManualInput ? (
            <>
              <label htmlFor='select-category'>
                Pick up an existing category from this list or{' '}
              </label>
              <button
                className='btn-like-link'
                onClick={handlePreferManualInputChange}>
                write a new one
              </button>
              <select
            ...
            <>
              <label htmlFor='add-new-category-input'>
                Write down a new category or{' '}
              </label>
              <button
                className='btn-like-link'
                onClick={handlePreferManualInputChange}>
          ...
          <button
            disabled={!newCategory || !newExpenseType}
            type='submit'
            onClick={onNewExpenseTypeAdded}>
            Add
          </button>
        ...
        {expenseTypes.length && (
          <ExpenseTypesList
            ...
            title='My own expense types'
            select={handleExpenseTypeRemove}
            type='remove'
          />
        )}
        {defaultExpenseTypes.length && (
          <ExpenseTypesList
            ...
            title='Suggested expense types'
            select={handleExpenseTypeAdd}
            type='add'
          />
        )}
