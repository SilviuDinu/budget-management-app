...
const AddExpensesPage = () => {
    ...
    const fetchPeopleWhoCanPay = async () => {
      try {
        ...
        if (response.data) {
          setPeopleWhoCanPay([
            ...peopleWhoCanPay,
            ...response.data.users?.map((person) => person.userName),
          ]);
        }
      } catch (err) {
        fetchedRef.current = true;
    ...
    try {
      ...
      if (response.data) {
        ...
        const canAddNewExpenseType =
          selectedExpenseType &&
          selectedCategory &&
          !expenseTypes.some((type) => isEqual(type, obj));
  ...
  const handleSelectExpenseName = (e) => {
    ...
    const selectedCat = expenseTypes.find(
  ...
  if (!expenseTypes.length) {
    return (
      <div className='container'>
        <h1>Oops...</h1>
        <h2>
          Sorry, you do not have any expense types associated with your account.
        </h2>
        <h2>
          Please go to{' '}
          <b>
            <Link to='/dashboard/add-expense-category'>
  ...
  return (
    <div className='container'>
      ...
      <div className='add-expense'>
        ...
        {showSuccessMessage && (
          <div className='success'>Successfully saved record!</div>
        )}
        {showErrorMessage && (
          <div className='fail'>
        ...
        {!manualInput ? (
          <label>
            ...
            <button
              ...
              onClick={(e) => setManualInput(!manualInput)}>
              Add a new one
            </button>
            <select
              onChange={handleSelectExpenseName}
          ...
          <div style={{ width: '100%' }}>
            Type a new expense name and choose a suited category from next list
            or{' '}
            <button
              className='btn-like-link'
              onClick={(e) => setManualInput(!manualInput)}>
